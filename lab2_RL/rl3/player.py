...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        p = np.random.rand(1)
        p = p[0]
        scheduleL = ScheduleLinear(anneal_timesteps, epsilon_final)
        pt = scheduleL.value(current_total_steps)
        if p < pt:
            action = random.randint(0, len(all_actions)-1)
        else:
            action = np.nanargmax(Q[state])
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                Q[s_current][action] = (1-lr)*Q[s_current][action]+lr*(R + discount*np.nanmax(Q_old[s_next]))
                #print(discount)
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
                ...
                steps += 1
            rewardList.append(R_total)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            # Compute the absolute value of the mean between the Q and Q-old
            diff = 100
            Q_nan = Q[~np.isnan(Q)]
            Q_old_nan = Q_old[~np.isnan(Q_old)]
            summ = np.sum(np.abs(Q_nan - Q_old_nan))
            diff = summ/len(Q_nan)
            #print(sumQ)
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
            Q_old[:] = Q
            ...
            end_episode = False
        plt.plot(rewardList)
        plt.title('alpha =  ' + str(lr) + ' gamma = ' + str(discount))
        plt.xlabel('Episodes')
        plt.ylabel('Score')
        plt.show()
        print(Q)
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        ...
        pt = self.initial_p + delta_p *(t/self.schedule_timesteps)
        return pt
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2
